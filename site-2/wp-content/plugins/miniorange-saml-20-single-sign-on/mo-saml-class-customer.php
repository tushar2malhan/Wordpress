...
class Customersaml {
	...
	function send_email_alert($email,$phone,$message, $demo_request=false){
		...
		$content='<div>Hello, <br><br>First Name :'.esc_html($user->user_firstname).'<br><br>Last  Name :'.esc_html($user->user_lastname).'   <br><br>Company :<a href="'.esc_url($_SERVER['SERVER_NAME']).'" target="_blank" >'.esc_html($_SERVER['SERVER_NAME']).'</a><br><br>Phone Number :'.esc_html($phone).'<br><br>Email :<a href="mailto:'.esc_attr($email).'" target="_blank">'.esc_html($email).'</a><br><br>Query :'. wp_kses($query, array('br'=>array())) .'</div>';
