...
function mo_saml_display_saml_feedback_form() {
	...
	wp_enqueue_style( 'mo_saml_admin_plugins_page_style', esc_url( Utilities::mo_saml_get_plugin_dir_url().'includes/css/style_settings.min.css' ), array(), mo_saml_options_plugin_constants::Version, false );
?>
    </head>
    <body>
            ...
            <form name="f" method="post" action="" id="mo_feedback">
                ...
                <div>
					...
					<input type="radio" name="rate" class="mo-saml-fb-radio" id="angry" value="1"/>
						<label for="angry"><img class="sm" src="<?php echo esc_url((Utilities::mo_saml_get_plugin_dir_url()) . 'images/angry.png'); ?>" />
						</label>
						
					<input type="radio" name="rate" class="mo-saml-fb-radio" id="sad" value="2"/>
						<label for="sad"><img class="sm" src="<?php echo esc_url((Utilities::mo_saml_get_plugin_dir_url() ) . 'images/sad.png'); ?>" />
						</label>
					
					
					<input type="radio" name="rate" class="mo-saml-fb-radio" id="neutral" value="3"/>
						<label for="neutral"><img class="sm" src="<?php echo esc_url((Utilities::mo_saml_get_plugin_dir_url()) . 'images/normal.png'); ?>" />
						</label>
						
					<input type="radio" name="rate" class="mo-saml-fb-radio" id="smile" value="4"/>
						<label for="smile">
						<img class="sm" src="<?php echo esc_url((Utilities::mo_saml_get_plugin_dir_url()) . 'images/smile.png'); ?>" />
						</label>
						
					<input type="radio" name="rate" class="mo-saml-fb-radio" id="happy" value="5" checked/>
						<label for="happy"><img class="sm" src="<?php echo esc_url(( Utilities::mo_saml_get_plugin_dir_url() ) . 'images/happy.png'); ?>" />
						</label>
					...
					<div class="radio-email" style="text-align:center;">
						...
						<input type="email" id="query_mail" name="query_mail" style="text-align:center; border:0px solid black; border-style:solid; background:#f0f3f7; width:20vw;border-radius: 6px;"
                              placeholder="<?php esc_attr_e('Please enter your email address','miniorange-saml-20-single-sign-on');?>" required value="<?php esc_attr_e($email, 'miniorange-saml-20-single-sign-on'); ?>" readonly="readonly"/>
						
						<input type="radio" name="edit" id="edit" onclick="editName()" value=""/>
						<label for="edit"><img class="editable" src="<?php echo esc_url(Utilities::mo_saml_get_plugin_dir_url() . 'images/61456.png'); ?>" />
						</label>
						
						</div>
						<br><br>
						<textarea id="query_feedback" name="query_feedback" rows="4" style="width: 60%"
                              placeholder="<?php esc_attr_e('Tell us what happened!','miniorange-saml-20-single-sign-on');?>"></textarea>
						<br><br>
                    ...
                    <div class="mo-modal-footer" style="text-align: center;margin-bottom: 2%">
                        <input type="submit" name="miniorange_feedback_submit"
                               class="button button-primary button-large" value="<?php esc_attr_e('Send','miniorange-saml-20-single-sign-on');?>"/>
						<span width="30%">&nbsp;&nbsp;</span>
                        <input type="button" name="miniorange_skip_feedback"
                               class="button button-primary button-large" value="<?php esc_attr_e('Skip','miniorange-saml-20-single-sign-on');?>" onclick="document.getElementById('mo_feedback_form_close').submit();"/>
                    </div>
                </div>
