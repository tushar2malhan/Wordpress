		...
		<ui id="mo_oauth_client_default_apps" class="mo-flex-container mo-wrap">';
			foreach($defaultappsjson as $appId => $application) {
				...
				if ( 'oauth2.1' === $appId ) {
					...
					echo '<li data-appid="'.esc_attr($appId).'" class="mo-flex-item"><a ' . ( "cognito" === $appId ? "id=vip-default-app" : "" ) . ' href="#" ><img class="mo_oauth_client_default_app_icon" src=" '. esc_url( plugins_url( '/partials/apps/images/'.$application->image, dirname(dirname(dirname(__FILE__ ))))).'">';
				}
