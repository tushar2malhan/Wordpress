...
class MOOAuth_Client_Admin_Guides {
	...
	public static function instructions_page($appname){
		$app = mooauth_client_get_app($appname);
		if(!$app) {
			...
			if(isset($currentapp)) {
				...
				if($currentapp['apptype'] === "oauth") {
					$app = mooauth_client_get_app('other');
				} else {
					$app = mooauth_client_get_app($currentapp['apptype']);
				}
			}
		}
		...
		if($app->guide !== "") {
			echo esc_attr('<li>Please follow the instruction given in Step by Step guide to get your Client ID, Client Secret and other necessary information. <a href="'.esc_attr($app->guide).'" target="_blank">Click here</a> to get the step by step guide to configure <i>'.explode('App', esc_attr($app->label))[0].'</i> as OAuth/OpenID Connect server.</li>');
		} else {
			echo '
