...
class IdToken implements IdTokenInterface
{
    ...
    {
		global $wpdb;
        $public_keys = $wpdb->get_row("SELECT * FROM ".$wpdb->base_prefix."moos_oauth_public_keys where client_id = '".$client_id."';", ARRAY_A);
        $algorithm = !empty($public_keys) ? $public_keys['encryption_algorithm'] : "HS256";
        if($algorithm === "RS256"){
            $private_key = $wpdb->get_row("SELECT * FROM ".$wpdb->base_prefix."moos_oauth_public_keys where client_id = '".$client_id."';", ARRAY_A)['private_key'];
        }
            ...
            $private_key = mo_oauth_server_decrypt( $result['client_secret'], $result['client_name'] );
        }
